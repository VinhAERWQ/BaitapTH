...
--cau 1--
go
CREATE FUNCTION fn_hangsx_masp (@masp VARCHAR(50))
RETURNS VARCHAR(50)
AS
BEGIN
...
go
go
SELECT dbo.fn_hangsx_masp('SP01')
go

--Cau 2--
go
CREATE FUNCTION Tonggiatrinhap(@namx INT, @namy INT)
RETURNS MONEY
AS
BEGIN
    ...
    FROM Nhap
    WHERE YEAR(ngaynhap) BETWEEN @namx AND @namy
    RETURN @TongGiaTriNhap
END
go
go
SELECT dbo.Tonggiatrinhap(2016,2019)
go


--Cau 3--
go 
CREATE FUNCTION tksoluongnhapxuat(@tenSanPham nvarchar(50), @nam int)
RETURNS int
AS
BEGIN
...
go
go
SELECT dbo.tksoluongnhapxuat('Galaxy Note 11',2020)
go

--Cau 4--
go
CREATE FUNCTION tonggiatrinhapngay(
    @ngay_bat_dau DATE,
    @ngay_ket_thuc DATE
)
RETURNS MONEY
...
go
go
SELECT dbo.tonggiatrinhapngay('2018-01-01', '2020-12-31') AS TongGiaTriNhap;
go

--Cau 5--
go
CREATE FUNCTION tonggiatrixuat(@tenHang NVARCHAR(20), @nam INT)
RETURNS MONEY
AS
BEGIN
...
go
go
SELECT dbo.tonggiatrixuat(N'Samsung', 2020) AS 'TongGiaTriXuat';
go

--Cau 6--
go
CREATE FUNCTION tksoluongnv(@tenPhong NVARCHAR(30))
RETURNS TABLE
AS
RETURN
...
go
go
SELECT * FROM tksoluongnv(N'Vật tư')
go

--Cau 7--
go
CREATE FUNCTION tktenspxuat(@ten_sp NVARCHAR(20), @ngay_xuat DATE)
RETURNS INT
AS
BEGIN
...
go
go
SELECT dbo.tktenspxuat('Samsung', '12-12-2020')
go

--cau 8--
go
CREATE FUNCTION sdtnvxuathoadon (@InvoiceNumber NCHAR(10))
RETURNS NVARCHAR(20)
AS
BEGIN
...
go
go
SELECT dbo.sdtnvxuathoadon('X01')
go

--cau9--
go
CREATE FUNCTION tongslgnhapxuat(@tenSP NVARCHAR(20), @nam INT)
RETURNS INT
AS
BEGIN
...
go
go
SELECT dbo.tongslgnhapxuat('Galaxy V21', 2020) AS TongNhapXuat;
go

--cau10--
go
CREATE FUNCTION tktongslgsphang(@tenhang NVARCHAR(20))
RETURNS INT
AS
BEGIN
    DECLARE @soluong INT;
...
go
go
SELECT dbo.tktongslgsphang('Samsung') AS 'Tổng số lượng sản phẩm của hãng'
go